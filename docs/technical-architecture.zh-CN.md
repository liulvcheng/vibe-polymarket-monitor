...
- 与相邻仓位之间空一行
- 用固定字段顺序输出
- 把 `Avg Price`、`Current Price`、`Value`、`Cost Basis`、`PnL` 和 `Change` 分开显示，避免信息挤在一行里
...
- 内容较短时，直接调用 `sendMessage`
- 内容过长需要拆分时，退化为单个文本附件发送，避免多段消息部分成功、部分失败
